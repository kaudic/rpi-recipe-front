...
import {
  requestFetchRecipesList,
  requestFetchDeleteRecipe,
  requestFetchPutRecipe,
  requestFetchPutImage,
  requestFetchCreateRecipe,
} from "../requests/recipesRequests";
    ...
    case FETCH_CREATE_RECIPE: {
      ...
      if (response.status === 200) {
        ...
        if (action.imgData) {
          ...
          store.dispatch(
            actionFetchPutImage(convertObjectToFormData(action.imgData)),
          );
        }
